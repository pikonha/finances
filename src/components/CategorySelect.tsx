...
export function CategorySelect({ categories, value, onChange, onCreate }: CategorySelectProps) {
  ...
  return <div className='space-y-2'>
    ...
    {isCreating && <div className='space-y-2 rounded-md border bg-muted/30 p-2'>
      <div className='flex gap-2'>
        ...
        />
        <Button type='button' size='icon' aria-label='Create category' disabled={!name.trim() || isSaving} onClick={() => void create()}>
          {isSaving ? <Plus className='size-4 animate-pulse' /> : <Check className='size-4' />}
        </Button>
        <Button type='button' size='icon' variant='ghost' aria-label='Cancel category creation' disabled={isSaving} onClick={cancel}>
          <X className='size-4' />
        </Button>
      </div>
